...
class SplineTraj(object):
    ...
    def sample(self, t):
        qsamp = np.array([self.splines[i].sample(t) for i in range(len(self.splines))])
    ...
    @staticmethod
    def generate(t_knots, q_waypts, qd_i=None, qd_f=None, qdd_i=None, qdd_f=None):
        qlen = len(q_waypts[0])
        if qd_i is None:
            qd_i=[0.]*qlen 
        if qd_f is None:
        ...
        assert(t_knots[0] == 0)
        assert(np.all((t_knots[1:]-t_knots[:-1]) > 0))
        assert(len(t_knots) == len(q_waypts))
