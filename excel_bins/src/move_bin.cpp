...
MoveBin::MoveBin() : group("excel") ,spinner(1)
{
	...
	//move_group_interface::MoveGroup grp("excel");
	//group = grp;
	//group = new move_group_interface::MoveGroup("excel");

	group.setPlanningTime(8.0);

	z_target = 0.1 + 0.875+0.127;
	dz = 0.2;
...
void MoveBin::detach_bin()
{
	...
	if (planning_scene.robot_state.attached_collision_objects.size()>0){
		std::ostringstream os;
		os << planning_scene.robot_state.attached_collision_objects[0].object.id<< " detached from the robot"; 
		ROS_INFO_STREAM(os);
